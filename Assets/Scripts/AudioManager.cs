...
public class AudioManager : MonoBehaviour
{
	...
	[Header("Custom Audio")]
	public AudioClip tntExplosionClip;
	public AudioClip crateBreaksClip;
	public AudioClip wumpaCollectClip;
	...
	public static void PlayHurtAudio()
	{
		...
		current.stingSource.clip = current.hurtClip;
		current.stingSource.Play();
	}
	...
	public static void PlayArrowAudio()
	{
		...
		current.stingSource.clip = current.arrowClip;
		current.stingSource.Play();
	}
	
	public static void PlayPlatformFalls()
	{
		//If there is no current AudioManager, exit
		if (current == null)
			return;

		current.stingSource.clip = current.platformClip;
		current.stingSource.Play();
	}
	
	
	public static void PlayDrownAudio()
	{
		//If there is no current AudioManager, exit
		if (current == null)
			return;

		current.stingSource.clip = current.drownClip;
		current.stingSource.Play();
	}
	
	public static void PlayChompAudio()
	{
		//If there is no current AudioManager, exit
		if (current == null)
			return;

		current.stingSource.clip = current.chompClip;
		current.stingSource.Play();
	}
	public static void PlayShieldCollectionAudio()
	{
		//If there is no current AudioManager, exit
		if (current == null)
			return;

		current.stingSource.clip = current.shieldCollectClip;
		current.stingSource.Play();
	}
	
	public static void PlayWumpaCollectionAudio()
	{
		//If there is no current AudioManager, exit
		if (current == null)
			return;

		current.stingSource.clip = current.wumpaCollectClip;
        current.stingSource.Play();
	}
	
	public static void PlayTntExplosionAudio()
	{
		//If there is no current AudioManager, exit
		if (current == null)
			return;

		current.stingSource.clip = current.tntExplosionClip;
		current.stingSource.Play();
	}
	
	public static void PlayHealthRecoverAudio()
	{
		//If there is no current AudioManager, exit
		if (current == null)
			return;

		current.stingSource.clip = current.recoverHealth;
		current.stingSource.Play();
	}
	
	public static void PlayCheckpointAudio()
	{
		//If there is no current AudioManager, exit
		if (current == null)
			return;

		current.stingSource.clip = current.checkpointClip;
		current.stingSource.Play();
	}
	
	public static void PlayBreakingCrateAudio()
	{
		//If there is no current AudioManager, exit
		if (current == null)
			return;

		current.stingSource.clip = current.crateBreaksClip;
		current.stingSource.Play();
	}
